...
function CalendarView({data,onCancel,onApptAdded}){
  ...
  // When single barber: anchor = Monday of week; when multi: anchor = any chosen day
  const startDay=isSingleBarber?(()=>{const mon=new Date(anchor);const day=mon.getDay();const diff=day===0?-6:1-day;mon.setDate(mon.getDate()+diff);return mon})():anchor
  const days=Array.from({length:numDays},(_,i)=>{const d=new Date(startDay);d.setDate(startDay.getDate()+i);return d})

  const goBack=()=>{const d=new Date(anchor);d.setDate(d.getDate()-(isSingleBarber?7:1));setAnchor(d)}
  const goFwd=()=>{const d=new Date(anchor);d.setDate(d.getDate()+(isSingleBarber?7:1));setAnchor(d)}
  const goToday=()=>setAnchor(new Date())

  const rangeLabel=isSingleBarber
  ...
  return<div style={{display:'flex',flexDirection:'column',height:'calc(100vh - 48px)'}}>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16,flexShrink:0,flexWrap:'wrap',gap:8}}>
      ...
      <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
        ...
        <Btn small variant="secondary" onClick={goBack}>←</Btn>
        <span style={{fontSize:13,fontWeight:600,minWidth:160,textAlign:'center'}}>{rangeLabel}</span>
        <Btn small variant="secondary" onClick={goFwd}>→</Btn>
      </div>
    </div>
    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
      <div className="fade" style={{background:'var(--white)',borderRadius:14,border:'1.5px solid var(--border)',padding:20,boxShadow:'var(--shadow)'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:3}}>
          {Object.entries(byHour).map(([h,c])=>{const pct=maxHour>0?(c/maxHour)*100:0;const isEmpty=c===0;return<div key={h} style={{display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:11,fontWeight:500,color:'var(--text3)',minWidth:36,flexShrink:0}}>{h}</span><div style={{flex:1,height:14,borderRadius:3,background:'var(--bg)',overflow:'hidden',minWidth:0}}><div style={{height:14,borderRadius:3,background:isEmpty?'transparent':pct>60?'var(--green)':pct>30?'var(--purple)':'var(--orange)',width:`${Math.max(pct,0)}%`,maxWidth:'100%',transition:'width .3s'}}/></div><span style={{fontSize:11,fontWeight:600,color:isEmpty?'var(--red)':'var(--text)',minWidth:16,textAlign:'right',flexShrink:0}}>{c}</span>{isEmpty&&<span style={{fontSize:10,color:'var(--red)',fontWeight:500,flexShrink:0}}>vacía</span>}</div>})}
        </div>
      </div>
